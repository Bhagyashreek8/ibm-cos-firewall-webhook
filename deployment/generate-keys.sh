...
# The certificate will be issued for the Common Name (CN) of `webhook-server.webhook-admin.svc`, which is the
...
# openssl req -x509 -nodes -newkey rsa:4096 -sha256 -keyout ca.key -out ca.crt -subj "/CN=Admission Controller Webhook Demo CA" -days 365
...
openssl req -new -key webhook-server-tls.key -subj "/CN=webhook-server.webhook-admin.svc" \
